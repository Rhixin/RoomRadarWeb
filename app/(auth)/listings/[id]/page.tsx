...
import { getSpecificListing } from "@/lib/listings";
import LoadingPlaceholder from "@/components/loading/loading";

const ListingDetails = () => {
  const [id, setId] = useState(null);
  const [listingInstance, setListingInstance] = useState(null);

  // Get the ID from the URL when the component mounts
  useEffect(() => {
    const currentUrl = window.location.pathname;
    const idFromUrl = currentUrl.split("/")[2]; // /listings/1
    setId(idFromUrl); // Set the ID state
  }, []);

  // Fetch listings from the API whenever `id` changes
  const fetchSpecificListing = async () => {
    if (id) {
      // Make sure ID is not null
      try {
        const response = await getSpecificListing(id);
        setListingInstance(response.data); // Set the listing data
      } catch (err) {
  ...
  // Trigger the API call when `id` is set
  useEffect(() => {
    if (id) {
      fetchSpecificListing(); // Fetch only when id is not null
    }
  }, [id]);
  ...
  return <>{listingInstance ? <><Navbar navbarType={2}></Navbar>
           ...
           {listingInstance.propertyName}
            ...
            <div className="flex flex-row text-sm gap-8">
              <div className="flex items-center gap-1">
                ...
                <span>
                  {listingInstance.address.street +
                    ", " +
                    listingInstance.address.barangay +
                    ", " +
                    listingInstance.address.municipality +
                    ", " +
                    listingInstance.address.province +
                    ", " +
                    listingInstance.address.country + " " + 
                    listingInstance.address.postalCode
                    }
          ...
          <div className="mr-10">
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-4">
                ...
                />
                  <div>
                    <h3 className="text-xl font-semibold">{listingInstance.landLord.profile.firstName + " " + listingInstance.landLord.profile.lastName}</h3>
                    <p className="text-gray-600">{listingInstance.landLord.profile.contactNumber}</p> 
                  </div>
              </div>
            ...
            <h1 className="text-m font-bold mt-3">House Rules</h1>
              {(JSON.parse(listingInstance.rulesJson)).map((rule, index) => (
      ...
      <div className="justify-between items-center">
          ...
          <div className="flex space-x-4 text-gray-600 text-sm">
            <div className="flex items-center space-x-2">
              <BsDoorOpenFill />
              <span>{listingInstance.numOfBeds} Rooms</span>
            </div>
            <div className="flex items-center space-x-2">
              <FaBed />
              <span>{listingInstance.numOfBedrooms} Beds</span>
            </div>
            <div className="flex items-center space-x-2">
              <FaShower />
              <span>{listingInstance.numOfBathrooms} Bathrooms</span>
            </div>
          </div>
        </div>
  ...
  </div></> : <LoadingPlaceholder></LoadingPlaceholder>}</>;
};
