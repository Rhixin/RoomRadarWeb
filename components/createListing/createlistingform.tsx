...
const CreateListingForm = ({ showModal, setShowModal }) => {
  ...
  const userId = userSession.data.account.id;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await createListing(data); // Ensure createListing is called

      if (response.success) {
        // Successful log in then redirect
        alert("Successfully Created a Listing!");
      } else {
        console.error("Response error:", response); // Log the entire response for debugging
        alert("Creating Listing failed. Response dili success");
      }
    } catch (error) {
      console.error("Create listing failed:", error); // Log the error details
          ...
          <div className="steps-container overflow-y-auto h-[80%] px-1 overflow-x-hidden">
            ...
            >
              <div className="form-group">
                <div>
                  <h3 className="font-bold flex-1 pt-5 mb-2">
                    Listing Details
                  </h3>
